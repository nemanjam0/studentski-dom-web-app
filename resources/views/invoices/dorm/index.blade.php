...
<table class="table table-sm">
  ...
  @foreach($dorminvoices as $invoice)
    <tr>   
      <th scope="row">{{$i++}}</th>
      ...
      <td>
      <form method="POST" action="{{ route('dorm.destroy',['dorm'=> $invoice->id]) }}">
      <a href="{{ route('dorm.edit',['dorm'=>$invoice->id]) }}" class="btn btn-primary">Edit</a>
      @method('DELETE')
      @csrf
