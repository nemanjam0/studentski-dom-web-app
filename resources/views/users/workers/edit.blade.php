...
<div class="container">
    ...
    <div class="row justify-content-center">
        <div class="col-md-8">
            <div class="card">
                <div class="card-header">{{ __('Edit worker') }}</div>

                <div class="card-body">
                <form method="POST" action="{{ route('workers.update',$data['id']) }}">
                        @method('PUT')
                        @csrf
                        <div class="form-group row">
                            ...
                            <div class="col-md-6">
                                ...
                                <option value="{{$worker->id}}" @if(($worker->id)==$data['dorm_worker']->user_id) selected="selected" @endif>{{$worker->name}}</option>
                                @endforeach
                                </select>
                                @error('user_id')
                        ...
                        <div class="col-md-6">
                                ...
                                <option value="{{$dorm->id}}" @if(($dorm->id)==$data['dorm_worker']->dorm_id) selected="selected" @endif>{{$dorm->name}} ({{$dorm->address}})</option>
                                @endforeach
                                </select>
                                @error('dorm_id')
                                    <span class="invalid-feedback" role="alert">
